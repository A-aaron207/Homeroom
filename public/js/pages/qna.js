      ...
      <div class="page-container page-qna fade-in">
        <div class="header-section" style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 2rem; flex-wrap: wrap; gap: 1rem;">
          <div>
            ...
            <p style="color: var(--text-muted); font-size: 1.1rem; margin-top: 0.5rem;">Get help and help others.</p>
          </div>
  ...
  async init() {
    ...
    document.getElementById('btn-ask-q')?.addEventListener('click', () => {
        ...
        document.getElementById('ask-q-form').addEventListener('submit', async (e) => {
            ...
            data.tags = JSON.stringify(data.tags.split(',').map(t=>t.trim()).filter(Boolean));
                ...
                ${qs.map(q => {
                    ...
                    return `
                        <div class="glass-card" style="padding: 1.5rem; display: flex; gap: 1.5rem; align-items: flex-start; cursor: pointer; transition: all 0.2s;" onclick="Homeroom.pages.qna.openQuestion('${q.id}')">
                            <div style="display: flex; flex-direction: column; align-items: center; min-width: 60px; background: rgba(0,0,0,0.2); padding: 1rem 0.5rem; border-radius: 0.5rem; border: 1px solid rgba(255,255,255,0.05);">
                                ...
                                <span style="font-size: 1.2rem; color: ${q.answer_count > 0 ? '#22c55e' : 'var(--text-muted)'}; font-weight: bold;">${q.answer_count}</span>
                            ...
                            <div style="flex: 1;">
                                ...
                                <div style="display: flex; justify-content: space-between; align-items: center;">
                                    <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                                        ${(JSON.parse(q.tags || '[]')).map(t => `<span style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); padding: 0.2rem 0.5rem; border-radius: 0.5rem; font-size: 0.75rem; color: var(--text-muted);">#${t}</span>`).join('')}
                                    </div>
                                    <div style="font-size: 0.85rem; color: var(--text-muted); display: flex; align-items: center; gap: 0.5rem;">
                                        <span>Asked by <span style="color: var(--accent-color);">${q.asked_by_name || 'User'}</span></span>
                                        <span>•</span>
                                        <span>${date}</span>
                                    </div>
                                </div>
    ...
    try {
        ...
        const renderAnswers = () => {
            ...
            return q.answers.map(a => `
                <div style="display: flex; gap: 1rem; padding: 1.5rem; background: rgba(0,0,0,0.2); border-radius: 0.5rem; border: 1px solid ${a.is_best ? 'rgba(34, 197, 94, 0.5)' : 'rgba(255,255,255,0.05)'}; position: relative; margin-bottom: 1rem;">
                    ${a.is_best ? '<div style="position: absolute; top: -10px; right: 20px; background: #22c55e; color: white; padding: 0.25rem 0.75rem; border-radius: 1rem; font-size: 0.8rem; font-weight: bold; box-shadow: 0 4px 10px rgba(34,197,94,0.3);">⭐ Best Answer</div>' : ''}
                    <div style="display: flex; flex-direction: column; align-items: center; min-width: 40px;">
                        <button class="btn-upvote-a" data-id="${a.id}" style="background: transparent; border: none; font-size: 1.5rem; cursor: pointer; color: var(--text-muted); transition: color 0.2s;">▲</button>
                    ...
                    <div style="flex: 1;">
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 1rem;">
                            <div style="font-size: 0.85rem; color: var(--text-muted);">
                                Answered by <span style="color: var(--accent-color);">${a.answered_by_name || 'User'}</span> on ${new Date(a.created_at).toLocaleDateString()}
                            </div>
                            ${isAuthor && !a.is_best ? `<button class="btn-mark-best" data-id="${a.id}" style="background: rgba(34,197,94,0.1); border: 1px solid rgba(34,197,94,0.3); color: #22c55e; padding: 0.4rem 0.8rem; border-radius: 0.5rem; cursor: pointer; font-size: 0.85rem; transition: all 0.2s;">Mark as Best</button>` : ''}
                        </div>
                    </div>
                </div>
            `).join('');
        ...
        Homeroom.modal.open(q.title, `
            <div style="display: flex; gap: 1rem; margin-bottom: 2rem;">
                <div style="display: flex; flex-direction: column; align-items: center; min-width: 40px;">
                    <button id="btn-upvote-q" style="background: transparent; border: none; font-size: 1.5rem; cursor: pointer; color: var(--accent-color); transition: transform 0.2s;">▲</button>
                ...
                <div style="flex: 1;">
                    ...
                    <div style="font-size: 0.85rem; color: var(--text-muted);">
                        Asked by <span style="color: var(--accent-color);">${q.asked_by_name || 'User'}</span> on ${new Date(q.created_at).toLocaleDateString()}
                    </div>
                </div>
            </div>
        ...
        document.querySelectorAll('.btn-mark-best').forEach(btn => {
            btn.addEventListener('click', async function() {
                ...
                try {
                    ...
                    Homeroom.toast('Marked as best answer!', 'success');
                    Homeroom.pages.qna.openQuestion(q.id);
                } catch(e) {}
            });
        });
